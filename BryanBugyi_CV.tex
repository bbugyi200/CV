...
\begin{itemize}
    \item
	\ressubheading{Bloomberg L.P.}{Manhattan, NY}{Site Reliability Engineer}{January 2021 - Current} \vspace{8pt}

        Site Reliability Engineering (SRE) at Bloomberg uses both software and systems engineering to build and reliably run their production systems.
...
\begin{itemize}
    ...
    \sub{Languages}\\
        I have assigned each of the languages listed below a rank between $R1-R5$ to indicate my level of proficiency with that language. I use ``lines-of-code ($loc$) written'' as my metric here (e.g. I have written $\geq5,000$ lines of code in all languages marked with a rank of $R2$ or above):\vspace{0.2cm}

        $R1: \geq 1,000$ $(loc)$\hspace{1.25cm}$R2: 5,000$\hspace{1.25cm}$R3: 20,000$\hspace{1.25cm}$R4: 50,000$\hspace{1.25cm}$R5: 150,000$\\\vspace{0.4cm}

        \declarecommand{\lang}{m m}{\textsc{#1}: #2}

        \ssub{Programming Languages}\vspace{0.25cm}

        \lang{Python}{$R5$}\hspace{1.0cm}
        \lang{Bash}{$R4$}\hspace{1.2cm}
        \lang{C/C++}{$R3$}\hspace{1.1cm}
        \lang{Perl}{$R2$}
        \vspace{0.2cm}

        \lang{Rust}{$R2$}\hspace{1.5cm}
        \lang{Haskell}{$R1$}\hspace{0.6cm}
        \lang{Java}{$R1$}\hspace{1.65cm}
        \lang{Javascript}{$R1$}

    \sub{Frameworks} Django, Flask, Pandas, Twisted
    \sub{Technologies} Debian, Docker, Git, PostgreSQL, RedHat, Vim
\end{itemize}
